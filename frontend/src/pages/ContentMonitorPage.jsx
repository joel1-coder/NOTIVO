...
/* Maps content row id → submission EVT id */
const contentToSubmissionMap = {
  1: "EVT-001", 2: "EVT-002", 3: "EVT-003", 4: "EVT-004",
  5: "EVT-005", 6: "EVT-006", 7: "EVT-007", 8: "EVT-008",
};
...
export default function ContentMonitorPage() {
  ...
  const handleDelete   = (id) => { setContent((prev) => prev.filter((c) => c.id !== id)); toast("Content removed."); };
  const updateStatus   = (id, newStatus) => { setContent((prev) => prev.map((c) => (c.id === id ? { ...c, status: newStatus } : c))); toast(`Status updated to ${newStatus}.`); };

  const handleView = (item) => {
    const submissionId = contentToSubmissionMap[item.id];
    if (submissionId) {
      const sub = submissions.find((s) => s.id === submissionId);
      if (sub) { setSelectedSubmission(sub); navigate("/invitation-detail"); return; }
    }
    toast("⚠️ Full details not available for this item.");
  };
              ...
              {filtered.map((c) => {
                ...
                return (
                  <tr key={c.id} className="es-tr">
                    ...
                    <td className="es-td">
                      <span className="task-status-badge" style={{ background:tBadge.bg, color:tBadge.color }}>{c.type}</span>
                    </td>
                    ...
                    <td className="es-td"><span className="cm-views-badge">👁 {c.views}</span></td>
                    <td className="es-td">
                      <select className="cm-status-select" style={{ background:sBadge.bg, color:sBadge.color }}
                    ...
                    <td className="es-td">
                      <div className="cm-actions-row">
                        <button className="cm-action-btn view-btn" title="View Full Details" onClick={() => handleView(c)}>
                          <EyeIcon />
                        </button>
                        <button className="cm-action-btn dl-btn" title="Download"><DownloadIcon /></button>
